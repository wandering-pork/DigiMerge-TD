...
### Enemy Count Formula
```typescript
function getEnemyCount(wave: number): number {
  const base = 6 + (wave * 0.5);
  const swarmBonus = wave > 5 ? (wave - 5) * 0.3 : 0;
  return Math.min(Math.floor(base + swarmBonus), 100);
}
```
...
function getSpawnInterval(wave: number): number {
  if (wave <= 10) return 2000;      // 2.0 seconds
  if (wave <= 20) return 1800;      // 1.8 seconds
  if (wave <= 40) return 1500;      // 1.5 seconds
  if (wave <= 60) return 1200;      // 1.2 seconds
  if (wave <= 80) return 1000;      // 1.0 seconds
  if (wave <= 100) return 800;      // 0.8 seconds
  return Math.max(300, 600 - (wave - 100) * 10);  // Endless: min 0.3s
}
```
...
function getEnemyStats(baseHP: number, baseDmg: number, wave: number, tier: number): EnemyStats {
  const phaseStarts = [1, 21, 41, 61, 81];
  const phaseStart = phaseStarts[tier];
  const wavesIntoPhase = wave - phaseStart;

  let hpMult = 1.0 + (0.08 * wavesIntoPhase);
  let dmgMult = 1.0 + (0.05 * wavesIntoPhase);

  // Endless scaling
  if (wave > 100) {
    hpMult *= Math.pow(1.05, wave - 100);
    dmgMult *= Math.pow(1.03, wave - 100);
  }
...
```typescript
function getModifierChance(wave: number): number {
  if (wave < 50) return 0.0;
  if (wave <= 60) return 0.2;
  if (wave <= 80) return 0.4;
  if (wave <= 100) return 0.6 + (wave - 80) * 0.02;
  return Math.min(1.0, 0.8 + (wave - 100) * 0.01);
}
```
