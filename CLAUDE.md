...
│   │   └── SpriteAtlasData.ts    # Auto-generated atlas mapping (155 entries)
│   ├── ui/
...
BootScene → PreloadScene → MainMenuScene → StarterSelectScene → GameScene
                              ↓    ↓                         ↓    ↓    ↓
                   EncyclopediaScene CreditsScene    PauseScene  SettingsScene  EncyclopediaScene
                                                                    ↓
                                                              GameOverScene
```
...
### Texture Atlases (5 per-stage atlases)
**Location**: `public/assets/sprites/atlases/`
- Generated by `scripts/generate-atlases.ts` (run via `npx tsx scripts/generate-atlases.ts`)
- 155 Digimon packed from sprite sheets (`assets/sprites/sprite sheets/`)
- Each Digimon has 12 frames (16x16 each) extracted from 48x64 sprite sheets
- Idle animation uses frames 0-2 (front-facing row) at 4fps
- `SpriteAtlasData.ts` maps game sprite keys → atlas entries (auto-generated)
- `SpriteAnimHelper.ts` provides runtime helpers for atlas-aware sprite display

| Atlas | Digimon | Frames | Size |
|-------|---------|--------|------|
| `atlas_baby2` | 44 | 528 | 368x368 |
| `atlas_child` | 99 | 1,188 | 560x544 |
| `atlas_adult` | 150 | 1,800 | 688x672 |
| `atlas_perfect` | 146 | 1,752 | 672x672 |
| `atlas_ultimate` | 176 | 2,112 | 736x736 |
...
| Status effects system | ✅ | Burn, Poison, Slow, Freeze, Stun, Armor Break, Armor Pierce, Holy, Heal |
| Tower skills display | ✅ | Effect name + proc chance in TowerInfoPanel |
| Floating damage numbers | ✅ | Color-coded by effectiveness + DoT colors (burn=orange, poison=purple), toggleable |
...
| Encyclopedia | ✅ | Browsable Digimon catalog with filters, pagination, detail view, in-game access |
