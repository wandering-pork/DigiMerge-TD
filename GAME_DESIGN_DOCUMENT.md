...
### Target Platform
Web (HTML5) via Phaser 3 + TypeScript, with optional desktop builds via Electron/Tauri
...
### DigimonData Interface
```typescript
// src/types/DigimonTypes.ts
interface DigimonData {
  id: string;
  name: string;
  stage: number;       // 0=In-Training, 1=Rookie, 2=Champion, etc.
  attribute: Attribute; // 0=Vaccine, 1=Data, 2=Virus, 3=Free
  family: string;
...
### EvolutionPath Interface
```typescript
interface EvolutionPath {
  resultDigimon: string;
  minDP: number;
  maxDP: number;
  isDefault: boolean;
}
...
class Tower extends Phaser.GameObjects.Sprite {
  ...
  public currentDP: number = 0;
  public currentLevel: number = 1;
  public originStage: number = 0;  // Stage when spawned

  private static BASE_MAX_LEVELS = [10, 20, 35, 50, 70, 100];
  private static DP_BONUSES = [1, 2, 3, 4, 5, 5];

  getMaxLevel(): number {
    const base = Tower.BASE_MAX_LEVELS[this.digimonData.stage];
    const dpBonus = Tower.DP_BONUSES[this.digimonData.stage];
    const originBonus = (this.digimonData.stage - this.originStage) * 5;
    return base + (this.currentDP * dpBonus) + originBonus;
  }

  canDigivolve(): boolean {
    const baseMax = Tower.BASE_MAX_LEVELS[this.digimonData.stage];
    return this.currentLevel >= baseMax;
  }

  getMaxReachableStage(): number {
    // Origin determines max stage
    switch (this.originStage) {
      case 0: return 2;  // In-Training origin → Champion max
      case 1: return 3;  // Rookie origin → Ultimate max
      case 2: return 4;  // Champion origin → Mega max
      default: return 5;
    }
  }
}
...
class GameStateManager {
  public digibytes: number = 200;
  public lives: number = 20;
  public currentWave: number = 1;
  public activeTowers: Tower[] = [];

  static readonly SPAWN_COSTS = {
  ...
  static readonly DIGIVOLVE_COSTS = [100, 150, 200, 250] as const;
...
### Project Structure (Phaser + TypeScript)

```
digimerge-td/
├── public/
│   └── assets/
│       ├── sprites/          # Digimon & enemy spritesheets
│       ├── ui/               # UI elements, buttons, panels
│       ├── effects/          # Particle effects, status icons
│       ├── audio/            # SFX and music
│       │   ├── sfx/
│       │   └── music/
│       ├── map/              # Tilemap and tileset
...
│   │
│   ├── scenes/
│   │   ├── BootScene.ts      # Asset key registration
│   │   ├── PreloadScene.ts   # Asset loading with progress
│   │   ├── MainMenuScene.ts  # Title screen, navigation
│   │   ├── StarterSelectScene.ts
│   │   ├── GameScene.ts      # Main gameplay
│   │   ├── PauseScene.ts     # Overlay pause menu
│   │   ├── GameOverScene.ts
...
│   │   └── Boss.ts           # Boss enemy with abilities
│   │
│   ├── managers/
│   │   ├── GameStateManager.ts    # DigiBytes, lives, wave, stats
│   │   ├── WaveManager.ts         # Enemy spawning, wave config
│   │   ├── CombatManager.ts       # Damage calculation, effects
│   │   ├── TowerManager.ts        # Placement, selection, merging
...
### Manager Pattern (Registry-based Singletons)

```typescript
// src/managers/GameStateManager.ts - Central game state
...
export class GameStateManager {
  ...
  // Methods emit events via EventBus
...
// src/utils/EventBus.ts - Decoupled communication
import Phaser from 'phaser';
...
| Desktop | `localStorage` via Electron/Tauri |

**Note:** Web builds use browser LocalStorage. For desktop builds using Electron or Tauri, the same LocalStorage API works with persistent storage.
...
export class SaveManager {
  static save(gameState: GameStateManager, towers: Tower[]): void {
    const saveData: SaveData = {
      version: VERSION,
      timestamp: new Date().toISOString(),
      gameState: {
        digibytes: gameState.digibytes,
        lives: gameState.lives,
        currentWave: gameState.currentWave,
        highestWave: gameState.highestWave,
        gameMode: gameState.gameMode,
        totalPlaytime: gameState.stats.playTime,
      },
      towers: towers.map(t => ({
        id: t.id,
        digimonId: t.digimonId,
        position: t.gridPosition,
        level: t.level,
        dp: t.dp,
        originTier: t.originTier,
  ...
  static exportSave(): string {
    ...
    return data ? btoa(data) : '';  // Base64 encode for sharing
  }
...
interface GameSettings {
  ...
  gameplay: {
    ...
    healthBars: 'all' | 'bosses' | 'off';
    rangeIndicators: boolean;
    tooltips: boolean;
    confirmMerge: boolean;
    confirmDigivolve: boolean;
    showTutorial: boolean;
    tutorialPromptAnswered: boolean;
  };
  accessibility: {
...
export const DEFAULT_SETTINGS: GameSettings = {
  ...
  gameplay: {
    ...
    tooltips: true,
    confirmMerge: true,
    confirmDigivolve: true,
    showTutorial: true,
    tutorialPromptAnswered: false,
  },
  accessibility: {
    colorblindMode: 'off',
    textSize: 'medium',
...
function shouldShowTutorialPrompt(): boolean {
  // Show prompt if player has completed Wave 10+ before
  const saveData = SaveManager.load();
  if (!saveData) {
    return false;  // First time player, tutorial ON by default
  }
  return saveData.gameState.highestWave >= 10;
}

function applyTutorialChoice(showTutorial: boolean, remember: boolean): void {
  const settings = SaveManager.getSettings();
  settings.gameplay.showTutorial = showTutorial;
  if (remember) {
    settings.gameplay.tutorialPromptAnswered = true;
  }
...
```typescript
// src/types/TutorialTypes.ts
interface TutorialHint {
  id: string;              // Unique ID like "wave3_attribute"
  wave: number;            // Which wave this hint belongs to (0 = any)
  trigger: TriggerType;    // Trigger type (see below)
  triggerData?: Record<string, any>;  // Additional conditions
  title: string;
  message: string;
  image?: string;          // Asset key for image
  highlightTarget?: string; // UI element ID to highlight
...
type TriggerType =
  ...
  | 'enemy_spawn'       // When specific enemy type spawns
  | 'enemy_damaged'     // When enemy takes attribute-effective damage
  | 'currency_threshold' // When DB reaches amount
  | 'tower_count'       // When player has X towers
  | 'merge_available'   // When player has 2+ same-stage same-attribute
  | 'boss_spawn';       // When boss enemy spawns

// src/managers/TutorialManager.ts
export class TutorialManager {
  private shownHints: Map<string, boolean> = new Map();
  private currentWave: number = 0;

  isTutorialEnabled(): boolean {
    const settings = SaveManager.getSettings();
    return settings.gameplay.showTutorial;
  }

  checkTrigger(trigger: TriggerType, data: Record<string, any> = {}): void {
    if (!this.isTutorialEnabled()) return;

    const hints = this.getHintsForTrigger(trigger, data);
    for (const hint of hints) {
      if (!this.shownHints.get(hint.id) || !hint.showOnce) {
        this.showHint(hint);
        if (hint.showOnce) {
          this.shownHints.set(hint.id, true);
          this.saveShownHints();
        }
      }
    }
  ...
  isTutorialComplete(): boolean {
    return this.currentWave > 10 || !this.isTutorialEnabled();
  }
  ...
  resetTutorial(): void {
    ...
    this.saveShownHints();
    const settings = SaveManager.getSettings();
    settings.gameplay.showTutorial = true;
    SaveManager.saveSettings(settings);
  }

  disableHint(hintId: string): void {
    this.shownHints.set(hintId, true);
    this.saveShownHints();
  }

  private saveShownHints(): void {
    localStorage.setItem('tutorial_progress', JSON.stringify([...this.shownHints]));
  }

  private loadShownHints(): void {
    const data = localStorage.getItem('tutorial_progress');
    if (data) {
      this.shownHints = new Map(JSON.parse(data));
    }
  }
}
```
